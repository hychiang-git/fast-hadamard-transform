...
@pytest.mark.parametrize("dtype", [torch.float32, torch.float16, torch.bfloat16])
@pytest.mark.parametrize("dim", [1, 2, 4, 8, 16, 32, 64, 128, 256, 512])
def test_fast_hadamard_transform_12N(dim, dtype):
    device = "cuda"
    N = 12
    rtol, atol = (3e-4, 3e-3) if dtype == torch.float32 else (3e-3, 5e-3)
    if dtype == torch.bfloat16:
    ...
    batch_size = 15
    # batch_size = 1
    x = torch.randn(batch_size, N*dim, device=device, dtype=dtype).requires_grad_()
    x_ref = x.detach().clone().requires_grad_()
    x_pt = x.detach().clone().requires_grad_()
    scale = 1 / math.sqrt(N*dim)
...
# @pytest.mark.parametrize("dtype", [torch.float32, torch.float16, torch.bfloat16])
# @pytest.mark.parametrize("dim", [1, 2, 4, 8, 16, 32, 64, 128, 256, 512])
# def test_fast_hadamard_transform_20N(dim, dtype):
#     device = "cuda"
#     N = 20
#     rtol, atol = (3e-4, 3e-3) if dtype == torch.float32 else (3e-3, 5e-3)
#     if dtype == torch.bfloat16:
...
# @pytest.mark.parametrize("dtype", [torch.float32, torch.float16, torch.bfloat16])
# @pytest.mark.parametrize("dim", [1, 2, 4, 8, 16, 32, 64, 128, 256, 512])
# def test_fast_hadamard_transform_28N(dim, dtype):
#     device = "cuda"
#     N = 28
#     rtol, atol = (3e-4, 3e-3) if dtype == torch.float32 else (3e-3, 5e-3)
#     if dtype == torch.bfloat16:
...
# @pytest.mark.parametrize("dtype", [torch.float32, torch.float16, torch.bfloat16])
# @pytest.mark.parametrize("dim", [1, 2, 4, 8, 16, 32, 64, 128, 256, 512])
# def test_fast_hadamard_transform_40N(dim, dtype):
#     device = "cuda"
#     N = 40
#     rtol, atol = (3e-4, 3e-3) if dtype == torch.float32 else (3e-3, 5e-3)
#     if dtype == torch.bfloat16:
...
# def test_fast_hadamard_transform_84N(dim, dtype):
#     device = "cuda"
#     N = 84
#     rtol, atol = (3e-4, 3e-3) if dtype == torch.float32 else (3e-3, 5e-3)
#     if dtype == torch.bfloat16:
